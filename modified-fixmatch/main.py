...
############################################################################
    checkpoint_path = './savedmodels/newcosinemodel_ckpt_{}_{}_{}.pt'.format(args.dataset, args.num_labeled, args.threshold, momentum = 0.9)
    best_model_path = './savedmodels/newcosinemodel_{}_{}_{}.pt'.format(args.dataset, args.num_labeled, args.threshold)
    start_epoch = 0
    best_accuracy = 0
    print("###")
    ...
    for epoch in range(start_epoch, args.epoch):
        for i in range(args.iter_per_epoch):
            ...
            #   temp = x_ul_weak
            #   x_ul_weak = x_ul_strong
            #   x_ul_strong = temp
            #   args.threshold = 0.95
            # elif(epoch>10):
            #   args.theshsold = temp_threshold
            ...
            output = model(x)
            intermediate_rep =activation['bn1']#
            inter_weak, inter_strong = intermediate_rep[batch_size:].chunk(2)       
            del intermediate_rep
            ...
            inter_mask = scores.ge(args.threshold)
            inter_weak = inter_weak[inter_mask]
            num_el = inter_weak.numel()
            if(num_el !=0 ):
                inter_strong = inter_strong[inter_mask]
                inter_weak = inter_weak.contiguous().view(-1,num_el)
                inter_strong = inter_strong.contiguous().view(-1,num_el)
                cosine_loss = F.cosine_embedding_loss(inter_weak, inter_strong, Variable(torch.Tensor(inter_weak.size(0)).to(device).fill_(1.0)))
            else:
                 cosine_loss = 0

            unlabeled_loss = (F.cross_entropy(output_u_strong, pseudo_targets,
                                  reduction='none') * mask).mean()
            total_loss = labeled_loss + alpha_weight(epoch) * (unlabeled_loss + cosine_loss)
